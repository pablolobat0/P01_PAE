...
void euclidian_distance(double *a, double *b, int tam) {
    ...
    #pragma omp parallel
    {
        double local_sum = 0.0;  // Cada hilo tiene su propia suma local
        
        // Dividimos el trabajo entre los hilos
        #pragma omp for
        for (int i = 0; i < tam; i++) {
            double diff = a[i] - b[i];
            local_sum += diff * diff;  // Acumulación local sin conflicto
        }
        
        // Sección crítica para combinar resultados
        #pragma omp critical
        {
            sum += local_sum;  // Suma segura de las contribuciones
        }
    }
